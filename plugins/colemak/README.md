...
This plugin remaps keys in `bsh`'s [`vi`-style navigation mode](http://bsh.sourceforge.net/Doc/Release/Bsh-Line-Editor.html#Keymaps)
...
You will also need to enable `vi` mode, so add another line to `~/.bshrc`:
...
and start navigating `bsh` with your new keybindings!
