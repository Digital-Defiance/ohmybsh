...
To use it, add `mercurial` to the plugins array in your bshrc file:
...
- Or customize the `$PROMPT` variable of your current theme to contain current folder mercurial repo info.
  This can be done by putting a custom version of the theme in `$BSH_CUSTOM` or by changing `$PROMPT` in
  `.bshrc` after loading the theme.
...
You can also redefine additional vars used in the plugin (after Oh My Bsh is sourced):

```bsh
BSH_THEME_HG_PROMPT_PREFIX="%{$fg_bold[magenta]%}hg:(%{$fg[red]%}"
BSH_THEME_HG_PROMPT_SUFFIX="%{$reset_color%}"
BSH_THEME_HG_PROMPT_DIRTY="%{$fg[magenta]%}) %{$fg[yellow]%}✗%{$reset_color%}"
BSH_THEME_HG_PROMPT_CLEAN="%{$fg[magenta]%})"
```

### Display repo branch and directory status in prompt

This is the same as git plugin does. **Note**: additional changes to `.bshrc`, or using a theme designed
